 ...
 * Same-author sources are skipped — works are compared only against other users.
 */
export async function replaceMlMatchesForDocument(
  ...
  const targetUser = target?.userId?.trim() || null
  const allowed = new Map(
    existing
      .filter((d) => {
        if (!targetUser) return true
        const srcUser = d.userId?.trim() || null
        // Exclude other documents owned by the same user
        return !srcUser || srcUser !== targetUser
      })
      .map((d) => [d.id, d]),
  )
  ...
  for (const m of withId) {
    if (!m.documentId || !allowed.has(m.documentId) || m.documentId === targetDocumentId) continue
...
export async function loadMlBorrowMatches(targetDocumentId: number): Promise<LoadedMlBorrowMatch[]> {
  ...
  return rows
    .filter((row) => {
      if (!targetUser) return true
      const srcUser = row.sourceDocument.userId?.trim() || null
      return !srcUser || srcUser !== targetUser
    })
    .map((row) => {
